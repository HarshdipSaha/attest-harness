...
| Smoke test | READY, awaiting go-ahead | Models switched to all-Groq (5 keys, rotated) per user decision — see `docs/DEVIATIONS.md` "Model lineup change". Config verified live: connectivity, tool-calling (`A4b_verified_tool`), and the judge all confirmed working end-to-end against the real API on 2026-09-10. Two real bugs found and fixed during this verification (judge silently defaulted every label to "partial"; tool schema 400'd on Groq) — see DEVIATIONS.md. 42/42 tests still pass. Not yet run: the actual 30-prompt dev-slice smoke test (`python -m attest_harness.runner --prompts prompts/dev_slice.csv --n 30 --only-model gpt_oss_120b --out results/smoke --cap 3`, times 3 models) — waiting on explicit go-ahead before spending any run volume. |
| Main run | TODO | after smoke test passes and PREREG is frozen |
